...
#subject 32 and subject 59 are the same person
#subject 42 and subject 100 are the same person
#They will be excluded because they took the same experiment twice

d <- read_delim("data_exp1_anonym.txt", delim = " ") %>%
  filter(ID !="subject_32", ID !="subject_59", ID !="subject_42", ID !="subject_100") %>% 
  filter(type == "Task", problem=="No")  %>% #keep subjects who did not encounter a technical problem during the experiment
  filter(score > 0.5)  #.. and got more than 50% correct on the training trials
