...
data_by_subj_s <- split(exp4_both_s, exp4_both_s$ID) 
data_by_subj_c <- split(exp4_both_c, exp4_both_c$ID)
data_by_subj_j <- split(exp4_both_j, exp4_both_j$ID)
...
coef_simu_s <- map2(model_by_subj_simu_s, ID, co_uni) %>%
  bind_rows() %>%
  rename(vrA_u= variance,
         bA_u =  bias,
         conv_A = convergence)
...
coef_simu_c <- map2(model_by_subj_simu_c, ID, co_uni) %>%
  bind_rows() %>%
  rename(vrV_u= variance,
         bV_u =  bias,
         conv_V = convergence)
...
coef_simu_j <- map2(model_by_subj_simu_j, ID, co_bim) %>%
  bind_rows() %>%
  rename(vrA_j = var_s,
         vrV_j = var_c,  
         b =  bias,
...
#feather::write_feather(weight, "ind_weights.feather")
#weight <- feather::read_feather("ind_weights.feather")


weight_simu_low <- quantile(weight_simu$prop, 0.05)
weight_simu_high <- quantile(weight_simu$prop, 0.95) 



ggplot(data=weight_both, aes(x=prop, y=..ncount..)) +
  #geom_histogram(data=subset(weight_both, data=='real'), fill="red", alpha=0.2, binwidth = 0.4)+
  geom_histogram(data=subset(weight_both, data=='simulated'), fill="blue", alpha=0.1, binwidth = 0.4)+
...
Simulat random responses from the population level model

First, take the real experimental data
```{r}
exp_exp4_s <- exp_good %>%
  filter(condition =="sound",
         experiment == "Experiment4")
...
Then, instead of the real answers, simulate new answers from the population-level model
