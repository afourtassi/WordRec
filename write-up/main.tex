...
%However, is it the case that children weight modalities differently because a sound is inherently more salient than any visual contrast? or could part of this effect stem from the fact that the precise sound and visual contrasts used as stimuli differ in their relative salience to children? In particular, what happens if the sound contrast is reduced to a perceptual minimum and when the visual contrast is enhanced? Can we reverse the weighting in this case as our optimal combination account would suggest? Moreover, could the developmental difference be driven by changes in the relative reliability of modalities?

%In a different line of work, it has been shown that infants have difficulties mapping minimally different words to two novel objects, despite the fact that they can perceptually differentiate these words \cite<e.g.,>{stager1997}. Is it possible that what had been interpreted as `failure' in learning could, at least to some extent, be explained by a categorical decision (similar to what adults performed in our task). More precisely, it is possible that the visual stimuli were quite similar in the children’s early conceptual space, which made them believe that both objects were possible instances of the same semantic category. This, in turn, would make them interpret the sound difference as alternative pronunciations of the same word. In particular, what happens if the object contrast is maximized? More work is needed to carefully examine these (and other) speculations, and more generally, to determine the extent to which the optimal combination account helps us better understand the mechanisms of word precessing and learning. 
