...
#Non-linear
sound_nls <- nls(answer ~ 1/(1+(1-e)*exp((-4/vrA)*sound_dist+(8/vrA))),
                 data=sound_all_exp1,
                 start = list(e=0, vrA=2))

#Non-linear mixed effect
sound_nlme <- nlme(answer ~ 1/(1+(1-e)*exp((-4/vrA)*sound_dist+(8/vrA))),
                 data=sound_all_exp1,
                 fixed = list(e + vrA ~ 1),
                 random = e+vrA ~ 1 | ID,
                 start = list(fixed =c(e=0, vrA=2)))

joint_nlme <- nlme(answer ~ 1/(1+exp((-4/vrA_j)*sound_dist+(-4/vrV_j)*concept_dist+(8/vrA_j)+(8/vrV_j))),
                   data=joint_all_exp1,
                   fixed = list(vrA_j + vrV_j ~ 1),
                   random = vrA_j ~ 1 | ID,
                   start = list(fixed =c(vrA_j=2, vrV_j=2)))

#Non-linear mixed effect (Bayesian inference)

prior0 <- prior(normal(2,5), nlpar = "vrA")#,
             #set_prior("normal(0, 1)", nlpar = "e"))
  
sound_brms <- brm(bf(answer ~ 1/(1+exp((-4/vrA)*sound_dist+(8/vrA))),
                     vrA ~ 1, nl=T),
                 data=sound_all_exp1,
                 prior = prior0, 
                 chains = 1,
...
Some previous research found a similar case of suboptimal behavior. For instance, studies that explored  the identification of ambiguous, newly learned pairs of word-referent associations all reported what appears to be a decrease in speech perception acuity in both children [@stager1997] and adults [@pajak2016].  Recently, @hofer2017 provided a probabilistic model of this phenomenon. In agreement with the method and finding in the current study, @hofer2017 characterized the apparent reduction in perceptual acuity as an increase in the noise variance of the auditory modality. Our finding, besides providing more evidence to this documented fact, suggests that the  reduction in perceptual acuity may occurs simultaneously in both the auditory *and* the visual modalities.
