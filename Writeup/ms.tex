...
different model for each participant. Thus, in Experiment 4, below, we
reran Experiment 1 while extending its length, allowing us to collect
the number of datapoints necessary to fit the models for each
participant.
...
because they reported having a technical problem, and N=10 participants
were excluded because they had less than 50\% accurate responses on the
unambiguous training trials. The final sample consisted of \(N =\) 88
participants.
...
difference was that we increased the number of responses elicited per
...
We analyzed cue combination at the individual level. For each
participant, we computed the optimal weighing,
\(\frac{\sigma^2_{A}}{\sigma^2_{V}}\), relative to the fitted (i.e.,
descriptive) weighing, \(\frac{\sigma^2_{Ab}}{\sigma^2_{Vb}}\). We show
the resulting distribution in Figure \ref{fig:weights}. We found that
the distribution centered around the optimal prediction (i.e., 1) with a
median value of 1.07 {[}\texttt{weight.median\_low},
\texttt{weight.median\_up}{]}.
...
combination optim\&lity, not only at the population level, but also at
...
distribution of cue combination scores had a unimodal shape
(approximating a normal distribution) centered around the optimal
combination. This finding rules out the hypotheis that optimality at the
population level (obtained in Experiment 1) is a spurious finding, i.e.,
only obtained via the aggregation of sub-otimal strategies. That said,
the large variance of this distribution indicates that some participants
tend to over-rely on the auditory modality and others tend to over-rely
on the visual modality (beyond measurement errors??).
