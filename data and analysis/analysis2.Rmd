...
d <- read_delim("data_experiment2_complete.txt", delim = " ") %>%
  filter(ID !="AR1IWBDA7MC86", ID !="A3GOKNZGZGHL45") %>% #took the experiment twice
  filter(type == "Task", problem=="No")  %>% #had a technical problem during the experiment
  filter(score > 0.5)  #got more less than 50% correct on the training trials
...
ggplot(joint, 
  ...
  geom_point(size=2) +
  geom_line(size=0.7) + 
  geom_line(data = sounds, aes(col = 1), col = "black",size=0.7) + 
